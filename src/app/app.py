...
@st.cache_data
def load_addresses():
    """Load danh sách địa chỉ từ dataset"""
    try:
        if data_path.exists():
...
if model is None:
    ...
    # Tabs cho thông tin model và đánh giá
    ...
    with tab2:
        ...
        # Load metrics từ file evaluation
            ...
            if metrics:
                ...
                with col_met1:
                    st.metric("RMSE", f"{metrics.get('RMSE', 0):.4f}", 
                             help="Root Mean Squared Error - càng thấp càng tốt")
                with col_met2:
                    st.metric("MAE", f"{metrics.get('MAE', 0):.4f}",
                             help="Mean Absolute Error - càng thấp càng tốt")
                with col_met3:
                    st.metric("MAPE", f"{metrics.get('MAPE', 0):.2f}%",
                             help="Mean Absolute Percentage Error - càng thấp càng tốt")
                with col_met4:
                    r2 = metrics.get('R2', 0)
                    st.metric("R² Score", f"{r2:.4f}",
                             help="R-squared - càng gần 1 càng tốt")
                    if r2 < 0.5:
                        st.warning("⚠️ R² thấp, model cần cải thiện")
                    elif r2 < 0.7:
                        st.info("ℹ️ R² ở mức trung bình")
                    else:
                        st.success("✅ R² tốt")
        ...
        with col_chart1:
            ...
            if scatter_path.exists():
                ...
                st.info("Biểu đồ scatter chưa có. Chạy evaluation để tạo.")
            ...
            if feature_imp_path.exists():
                ...
                st.info("Biểu đồ feature importance chưa có.")
        ...
        with col_chart2:
            ...
            if residuals_path.exists():
                ...
                st.info("Biểu đồ residuals chưa có. Chạy evaluation để tạo.")
            
            corr_path = evaluation_dir / "correlation_matrix.png"
            if corr_path.exists():
                st.image(str(corr_path), caption="Correlation Matrix")
            else:
                st.info("Biểu đồ correlation chưa có.")
        
        # Giải thích metrics
        with st.expander("ℹ️ Giải thích Metrics"):
            st.markdown("""
            **RMSE (Root Mean Squared Error)**: 
            - Đo lường độ lệch trung bình của dự đoán so với giá trị thực tế
